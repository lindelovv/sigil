...
draw_ui :: proc(cmd: vk.CommandBuffer, img_view: vk.ImageView) {
    ...
    imgui.Begin("performance", nil, { .NoTitleBar, .NoBackground, .NoResize, .NoMove, .NoCollapse, .NoMouseInputs })
    {
        ...
        imgui.TextUnformatted(fmt.caprintf("fps: %.0f", fps, allocator = context.temp_allocator))
        imgui.TextUnformatted(fmt.caprintf("ms:  %.3f", ms, allocator = context.temp_allocator))
        imgui.TextUnformatted("")
        imgui.TextUnformatted(fmt.caprintf("fwd:   %.3f", cam.forward, allocator = context.temp_allocator))
        imgui.TextUnformatted(fmt.caprintf("up:    %.3f", cam.up, allocator = context.temp_allocator))
        imgui.TextUnformatted(fmt.caprintf("right: %.3f", cam.right, allocator = context.temp_allocator))
        imgui.TextUnformatted("")
