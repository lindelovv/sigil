...
void old_renderer::init() {
    ...
    {
        std::vector<char> vertex_code        = read_file("./shaders/default.vert.spv");
        std::vector<char> fragment_code      = read_file("./shaders/default.frag.spv");
...
old_renderer::QueueFamilyIndices old_renderer::find_queue_families(vk::PhysicalDevice phys_device) {
...
vk::SurfaceFormatKHR old_renderer::choose_swap_surface_format(const std::vector<vk::SurfaceFormatKHR>& available_formats) {
...
vk::PresentModeKHR old_renderer::choose_swap_present_mode(const std::vector<vk::PresentModeKHR>& available_present_modes) {
...
vk::Extent2D old_renderer::choose_swap_extent(const vk::SurfaceCapabilitiesKHR& capabilities) {
...
void old_renderer::cleanup_swap_chain() {
...
void old_renderer::create_swap_chain() {
...
void old_renderer::recreate_swap_chain() {
...
vk::ImageView old_renderer::create_img_view(vk::Image image,
                                        vk::Format format,
...
void old_renderer::create_img_views() {
...
std::vector<char> old_renderer::read_file(const std::string& path) {
...
vk::ShaderModule old_renderer::create_shader_module(const std::vector<char>& code) {
...
void old_renderer::create_buffer(vk::DeviceSize size,
...
void old_renderer::update_uniform_buffer(uint32_t current_image) {
...
void old_renderer::copy_buffer(vk::Buffer src_buffer,
...
uint32_t old_renderer::find_memory_type(uint32_t type_filter, vk::MemoryPropertyFlags properties) {
...
void old_renderer::create_img(uint32_t width,
                          uint32_t height,
                          uint32_t mip_levels,
...
void old_renderer::transition_img_layout(vk::Image img,
                                     vk::Format format,
...
void old_renderer::copy_buffer_to_img(vk::Buffer buffer,
                                  vk::Image img,
                                  uint32_t width,
                                  uint32_t height
) {
...
void old_renderer::generate_mipmaps(vk::Image image,
...
void old_renderer::create_depth_resources() {
...
vk::Format old_renderer::find_supported_format(const std::vector<vk::Format>& candidates,
                                           vk::ImageTiling tiling,
...
bool old_renderer::has_stencil_component(vk::Format format) {
    return format == vk::Format::eD32SfloatS8Uint
        || format == vk::Format::eD24UnormS8Uint;
}

vk::Format old_renderer::find_depth_format() {
...
void old_renderer::create_color_resources() {
...
vk::CommandBuffer old_renderer::begin_single_time_commands() {
...
void old_renderer::end_single_time_commands(vk::CommandBuffer command_buffer) {
...
void old_renderer::create_framebuffers() {
...
void old_renderer::record_command_buffer(vk::CommandBuffer command_buffer, uint32_t img_index) {
    vk::CommandBufferBeginInfo begin_info {
        .flags = vk::CommandBufferUsageFlagBits::eOneTimeSubmit,
    };
...
#ifdef _DEBUG
VkResult old_renderer::create_debug_util_messenger_ext(VkInstance instance,
...
void old_renderer::destroy_debug_util_messenger_ext(VkInstance instance,
                                                VkDebugUtilsMessengerEXT p_debug_messenger,
                                                const VkAllocationCallbacks* p_allocator
) {
...
VKAPI_ATTR VkBool32 VKAPI_CALL old_renderer::debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT msg_severity,
