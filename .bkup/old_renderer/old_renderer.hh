...
struct old_renderer {
    old_renderer() {
        schedule(runlvl::init, [&]{ this->init();      });
        schedule(runlvl::tick, [&]{ this->draw();      });
        schedule(runlvl::exit, [&]{ this->terminate(); });
    }
    void init();
    void terminate();
    void draw();
...
namespace std {
    template<> struct hash<old_renderer::Vertex> {
        size_t operator()(old_renderer::Vertex const& vertex) const {
